...
class _PasswordDetailPageState extends State<PasswordDetailPage> {
  ...
  void initState() {
    ...
    passwordTextController.addListener(onPasswordListener);
  }
  ...
  void dispose() {
    ...
    passwordTextController.removeListener(onPasswordListener);
        ...
        body: CustomFutureBuilder(
            ...
            onSuccess: (AsyncSnapshot asyncSnapshot) {
              ...
              if(item != null) {
                ...
                passwordStrength = estimatePasswordStrength(passwordTextController.text);
                ...
                return ListView(
                  children: [
                        ...
                        children: [
                          Chip(
                            avatar: CustomCircleAvatar(
                              ...
                              child: Text('id', style: TextStyle(fontSize: AppResponsive.secondryFontSize.resolve(context).sp)),),
                            label: Text('${item.id}', style: TextStyle(fontSize: AppResponsive.primaryFontSize.resolve(context).sp)),
                          ),
                          Chip(
                            avatar: CustomCircleAvatar(
                              maxRadius: AppResponsive.circleRadius.resolve(context).w,
                              child: Text(context.watch<AppConfig>().userFirstName, style: TextStyle(fontSize: AppResponsive.secondryFontSize.resolve(context).sp)),),
                            label: Text(context.watch<AppConfig>().userLastName, style: TextStyle(fontSize: AppResponsive.primaryFontSize.resolve(context).sp)),
                          ),
                        ],
                      ),
                    ),
                    ...
                    ListTile(
                      dense: true,
                      contentPadding: EdgeInsetsDirectional.only(start: 16.0, end: 24.0, top: 0, bottom: 0),
                      title: CustomPasswordStrengthEstimator(
